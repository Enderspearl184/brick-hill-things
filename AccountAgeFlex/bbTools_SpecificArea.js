...
function btools(player) {
	...
	if (player.btools) {
		...
		player.buildcore.setCollision(false)
	...
	p.buildcore.setCollision(false)
    })
	...
	if (p.PlacedBrickCount >= 250 && p.userId!==305122 && !Game.local) {
		brick.destroy()
	...
	p.topPrint(p.PlacedBrickCount + "/250 bricks placed.", 7)
...
Game.command("btools", (p) => {
	...
	return console.log("removing btools from " + p.username + ".")
})