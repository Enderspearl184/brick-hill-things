...
Game.on("playerJoin", (player) => {
    ...
    player.on("initialSpawn", async() => {
		...
		brick.setInterval(async() => {
			...
			} else if (brick.model!==0 && isNaN(player.model)) {
	...
	player.setInterval(() => {
		if (player.toolEquipped && player.buildcore && player.showBrickTools.includes(player.toolEquipped)) {
			player.buildcore.setVisibility(0.5)
		} else  if (player.buildcore) {
			player.buildcore.setVisibility(0)
		}
	}, 100)
	player.keypress((key) => {
		if (key==="e") {
			if (player.brickplacement >= 7) {
				player.brickplacement = 7
				return player.message(`[#00ff00][Height]: [#ffffff]You increased the brick placement height to ${player.brickplacement}.`)
			}
			player.brickplacement++
			player.message(`[#00ff00][Height]: [#ffffff]You increased the brick placement height to ${player.brickplacement}.`)
		} else if (key==="q") {
			if (player.brickplacement <= -7) {
				player.brickplacement = -7
				return player.message(`[#ff0000][Height]: [#ffffff]You decreased the brick placement height to ${player.brickplacement}.`)
			}
			player.brickplacement--
			player.message(`[#ff0000][Height]: [#ffffff]You decreased the brick placement height to ${player.brickplacement}.`)
		} else if (key==="r") {
			player.brickrot+=45
			player.brickrot%=360
			console.log(player.brickrot)
			player.message(`[#00d0ff][Rotate]: [#ffffff]You changed the brick rotation to ${player.brickrot}.`)
		}
	})
})
...
Game.command("controls", (p) => {
	p.message("Use the number keys to select tools")
	p.message("Click with a tool to use it.")
	p.message("Press E to increase brick height and Q to decrease it.")
	p.message("Press R to change the brick rotation.")
	p.message("To destroy bricks, select the Destroy tool and click the unwanted brick.")
})
...
function btools(player) {
	...
	create.on("activated", async(p) => {
		...
		Game.newBrick(brick)
		await sendClickable(false, brick, p)
		Game.newBrick(brick)
    ...
    player.showBrickTools=[]
    player.showBrickTools.push(create)
    player.showBrickTools.push(sizeInc)
    player.showBrickTools.push(sizeDec)
}
...
async function destroyBrick(brick, player, all, caller) {
	...
	for (let deletethese of deletebricks) {
		...
		let bricks = []
		deletethese.setPosition(new Vector3(99999,99999,99999))
		await sleep(10)
		deletethese.setVisibility(0)
