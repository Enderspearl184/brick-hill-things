...
function btools(player) {
	...
	create.on("activated", p => {
		...
		if (!isInBounds(p, brick)) return brick.destroy()
		...
		if (p.PlacedBrickCount >= 500 && p.userId!==305122 && !Game.local && !p.ownsAsset(147324)) {
			brick.destroy()
			return p.message("You have reached the max brick count. Use the destroy tool, or do /clear to clear them!")
		}
		if (p.PlacedBrickCount >= 1000 && p.userId!==305122 && !Game.local && p.ownsAsset(147324)) {
			brick.destroy()
			return p.message("You have reached the max brick count. Use the destroy tool, or do /clear to clear them!")
		}
		...
		if (!p.ownsAsset(147324) && !Game.local) p.topPrint(p.PlacedBrickCount + "/500 bricks placed.", 7)
		if (p.ownsAsset(147324) || Game.local) p.topPrint(p.PlacedBrickCount + "/1000 bricks placed", 7)
		Game.newBrick(brick)
...
async function destroyBrick(brick, player, all) {
	...
	if (brickcount==0) {
	Game.deleteBricks(deletebricks)
	return "finished"
}
	...
	fs.readFile(`./playerData/${userId}_slot${slot}.sav`, async function(err, data) {
		...
		//put bricks in game and add them, if they are in bounds.
			...
			if (isInBounds(p, brick)) {
				...
				//if brick can be placed wait 10ms to prevent lag
				await sleep(10)
...
function brickClick(p, brick) {
    if (p.toolEquipped==p.destroytool && brick.placedby==p.username && !brick.destroying) {
		...
		if (p.PlacedBrickCount<0) p.prompt("PlacedBrickCount is less than 0. Message Enderspearl184")
			p.prompt("PlacedBrickCount is less than 0. Message Enderspearl184 with details on what you did to cause this.")
			p.PlacedBrickCount=p.bricks.length
		}
		...
		p.placedBrickCount=p.bricks.length
		if (!p.ownsAsset(147324) && Game.local==false) p.topPrint(p.PlacedBrickCount + "/500 bricks placed.", 7)
		if (p.ownsAsset(147324) || Game.local) p.topPrint(p.PlacedBrickCount + "/1000 bricks placed", 7)
	}
}