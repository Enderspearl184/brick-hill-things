...
//finished:
// client packets
// all server packets except brick
	...
	for (let i = 0; i < count; i++) {
		...
		if (attributes) {
			for (let i = 0; i < attributes.length; i++) {
				...
				switch (ID) {
					case "A":
						brickObj.rotation=reader.readUInt32LE();
						break;
					case "B":
						brickObj.shape=reader.readStringNT();
						break;
					case "C":
						brickObj.model=reader.readUInt32LE();
						break;
					case "D": //light
						reader.readUInt32LE()
						reader.readUInt32LE()
...
async function serverPlayerModificationPacket(socket,reader){
	...
	//WeatherRain
	//WeatherSnow
	//kick (ignored it closes the socket anyways)
	//prompt
...
async function serverBrickPacket(socket,reader){
	...
	switch (packetObj.data.action) {
		...
		case 'clickable':{
			...
			packetObj.data.clickDistance=reader.readUInt32()
			socket.queue.push(packetObj)
			break
		}
	}
...
async function handleSocket(req,res,body) {
	...
	if (sockets[socketid]) {
		...
		res.writeHead(200)
		res.end(JSON.stringify({queue:sockets[socketid].queue,status:"ok"}))
...
http.createServer(async function (req,res){
	...
	if (req.method!=="POST") {
		res.writeHead('405');
		res.end('{"status":"methodNotAllowed", "message":"Use a POST request for the urls dummy"}')
		return
	}
	if (req.url==!"/createSocket" && !req.url.startsWith('/sockets')) {
		res.writeHead('404')
		res.end('{"status":"notFound", "message":"Wrong url dumb stupid head"}')
		return
	}
	let body='';
	req.on('data',(chunk)=> {
...
createSocket(42482,'localhost','local')