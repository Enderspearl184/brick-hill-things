...
var UserInfo = {
	...
	GetGameVisits: async function (p) {
		...
		//removes most characters before game visit integer
		let string2 = string1.replace(/(.)*id="game-/m, '')

		//removes "," because it messes the result up if user has more than 999 visits
		let string3 = string2.replace(/,/g, '')
	
		//gets the visit count
		let gameVisits = string3.match(/\d+/).shift();

		p.gameVisits = parseInt(gameVisits)
		return p.gameVisits
	}
