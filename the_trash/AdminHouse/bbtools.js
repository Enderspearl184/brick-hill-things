...
function btools(player) {
	...
	Game.newBrick(brick)
	brick.clicked(debounce((player) => {
    		if (player.toolEquipped==player.destroytool) {
			console.log(player.username, "destroyed a brick")
		 	destroyBrick(brick)
		 }
	}), 500)
    })
...
//		await sleep(300)
//		if (brick.name == "btools" && brick.placedby==player.username) destroyBrick(brick)
//	})
//})
...
Game.command("clearall", (player) => {
	world.bricks.forEach(async(brick) => {
		if (player.userId!==305122) return
		await sleep(300)
		if (brick.name == "btools") destroyBrick(brick)
	})
})

async function destroyBrick(brick) {
	brick.destroy()
}