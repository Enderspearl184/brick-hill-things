...
var UserInfo = require('./../user_scripts/UserInfoModule.js')
...
Game.on("playerJoin", player => {
    player.on("initialSpawn", async() => {
        player.frozen = false;
	player.ownsTShirt = await ownsVIPShirt(player, 147324)
...
Game.command("size", (p, m) => {
    if (!(isAdmin(p.userId) || p.ownsTShirt)) return p.message("You need to own https://www.brick-hill.com/shop/147324 to do that.")
...
async function ownsVIPShirt (p, id) {
	await UserInfo.GetCrate(p, "tshirt")
	for (let tshirts of p.crate) {
		if (tshirts.item_id==id) return true
	}
        return false
}