...
//UserInfo.GetGameVisits(player) - get the user's game visit count.
...
var UserInfo = {
	...
	GetForumPosts: async function (p) {
		...
		for (let player of Game.players) {
			if (player==p) PlayerExists = true
		}
		if (PlayerExists!==true) return console.error("Player doesn't exist")
		let PostCount = await getUserPage(p.userId, "(.)*id=\"foru")
		p.forumPosts = PostCount
		return p.forumPosts
	},

	GetGameVisits: async function (p) {
		var PlayerExists = false
		for (let player of Game.players) {
			if (player==p) PlayerExists = true
		}
		if (PlayerExists!==true) return console.error("Player doesn't exist")
		let Visits = await getUserPage(p.userId, "(.)*id=\"game-")
		p.gameVisits = Visits
		return p.gameVisits
	}
...
async function doSort(p, sort, page) {
	...
	for (let item of JSONPageData.data) {
		...
		if (!p.crate.find((item) => item.item_id==itemObj.item_id)) {
...
async function getUserPage(id, regex) {
		//gets user page
		let userProfile = await phin("https://www.brick-hill.com/user/" + id)
		...
		var string2regex = new RegExp(regex, "m")
		let string2 = string1.replace(string2regex, '')

		//removes "," because it messes the result up if user has more than 999 posts
		let string3 = string2.replace(/,/g, '')
	
		//gets the stat number
		let Stat = string3.match(/\d+/).shift();
